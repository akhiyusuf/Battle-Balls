...
export const ROSTER: CharacterDef[] = [
  {
    ...
    hp: 165, speed: 285, radius: 42, weapon: { kind: "blade", length: 240, width: 26, style: "greatsword" },
    spin: 6, damage: 9.5,
    ult: {
    ...
    onDealHit(f) { f.st.damage += 0.35; f.st.spinMult = f.st.damage / (2 * 2.5); }, // Damage = 2x Spin Speed
    stats: (f) => [
...

  {
    ...
    hp: 202, speed: 266, radius: 45, weapon: { kind: "blade", length: 215, width: 30, style: "axe" },
    spin: 5, damage: 7.5,
    ult: {
...

  {
    ...
    hp: 142, speed: 294, radius: 40, weapon: { kind: "gun", length: 130, width: 36, style: "revolver" },
    spin: 0, damage: 6.6,
    ult: {
      name: "HIGH NOON", gainDealt: 0.55, gainTaken: 0.45,
      fire(f, e) {
        ...
        for (let i = 0; i < 6; i++) {
          e.schedule(0.12 + i * 0.14, () => {
            ...
            e.spawnProjectile(f, "bullet", mx, my, a,
              { speed: 1710, dmg: f.st.damage * 1.4, r: 9, color: 0xffd94d, life: 2 });
        ...
        if (f.st.ammo > 0) {
          ...
          if (t) {
            ...
            e.spawnProjectile(f, "bullet", f.x + Math.cos(a) * (f.def.radius + 26), f.y + Math.sin(a) * (f.def.radius + 26), a,
              { speed: 1045, dmg: f.st.damage, r: 8, color: 0xffd94d, life: 2 });
            Sound.pew();
            f.st.ammo--;
            f.st.shotT = 1.9;
          }
...

  {
    ...
    hp: 135, speed: 276, radius: 40, weapon: { kind: "blade", length: 155, width: 22, style: "wand" },
    spin: 3.6, damage: 4.1,
    ult: {
        ...
        for (let i = 0; i < 10; i++) {
          e.schedule(0.2 + i * 0.09, () => {
            ...
            e.spawnProjectile(f, "heart", f.x + Math.cos(a) * (f.def.radius + 16), f.y + Math.sin(a) * (f.def.radius + 16), a,
              { speed: 456, dmg: f.st.damage * 1.8, r: 15, color: 0xf284c1, life: 4, turn: 2.6 });
            Sound.magic();
          });
        }
      },
    ...
    update(f, e, dt) {
      ...
      if (f.st.castT <= 0) {
        ...
        if (t) {
          ...
          e.spawnProjectile(f, "heart", f.x + Math.cos(a) * (f.def.radius + 16), f.y + Math.sin(a) * (f.def.radius + 16), a,
            { speed: 494, dmg: f.st.damage * 1.6, r: 14, color: 0xf284c1, life: 4, turn: 2.3 });
          Sound.magic();
        }
        f.st.castT = 3.2;
      }
...

  {
    ...
    hp: 158, speed: 289, radius: 41, weapon: { kind: "staff", length: 330, width: 24, stripes: 6 },
    spin: 5.8, damage: 4.4,
    ult: {
...

  {
    ...
    hp: 150, speed: 270, radius: 41, weapon: { kind: "blade", length: 205, width: 26, style: "iceblade" },
    spin: 5.4, damage: 5.5,
    ult: {
...

  {
    ...
    hp: 128, speed: 408, radius: 37, weapon: { kind: "none" },
    spin: 0, damage: 9,
    ult: {
    ...
    update(f, _e, dt) {
      ...
      f.st.damage += dt * 0.16;
      f.st.contact = f.st.damage * (f.ultT > 0 ? 3 : 2.1);
      f.st.trail = 1;
...

  {
    ...
    hp: 262, speed: 209, radius: 52, massMult: 3,
...

  {
    ...
    hp: 150, speed: 300, radius: 40, weapon: { kind: "blade", length: 235, width: 20, style: "katana" },
    spin: 6.4, damage: 8.5,
    ult: {
    ...
    onDealHit(f) { f.st.damage += 0.3; f.st.spinMult = Math.min(2.2, (f.st.spinMult ?? 1) + 0.05); },
...

  {
    ...
    hp: 1, speed: 285, radius: 44, weapon: { kind: "blade", length: 205, width: 28, style: "gavel" },
    spin: 3.8, damage: 6.5,
    ult: {
...

  {
    ...
    hp: 135, speed: 323, radius: 38, weapon: { kind: "orbitals", count: 3, radius: 17, dist: 74 },
    spin: 6.4, damage: 3.2,
    ult: {
    ...
    modDamage(f, base, e) {
      const crit = e.rng.next() * 100 < (f.st.crit ?? 20);
      return crit ? base * 3 : base * 1.3;
    },
    onDealHit(f) { f.st.crit = Math.min(80, (f.st.crit ?? 20) + 2.2); },
    stats: (f) => [{ label: "Crit %", value: () => f2(f.st.crit ?? 20) }],
  },

  {
    id: "duelist", name: "Duelist", color: 0x5f6b76, dark: 0x232a31, theme: "plain",
    hp: 150, speed: 304, radius: 39, weapon: { kind: "blade", length: 225, width: 16, style: "rapier" },
    spin: 5.4, damage: 5,
    ult: {
      ...
      fire(f, e) {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          e.spawnProjectile(f, "bullet", f.x + Math.cos(a) * (f.def.radius + 20), f.y + Math.sin(a) * (f.def.radius + 20), a,
            { speed: 855, dmg: f.st.damage * 1.3, r: 7, color: 0xd8dce4, life: 1.6 });
        }
      },
    },
...

  {
    ...
    hp: 172, speed: 281, radius: 42, weapon: { kind: "blade", length: 260, width: 28, style: "chainsaw" },
    spin: 5, damage: 9,
    ult: {
...

  {
    ...
    hp: 158, speed: 262, radius: 42, weapon: { kind: "none" },
    spin: 0, damage: 4,
    ult: {
    ...
    update(f, e, dt) {
      ...
      if (f.st.fireT <= 0) {
        ...
        if (t) {
          ...
          e.spawnProjectile(f, "nail", f.x + Math.cos(a) * (f.def.radius + 30), f.y + Math.sin(a) * (f.def.radius + 30), a,
            { speed: 998, dmg: f.st.damage * 2.2, l: e.rng.range(280, 420), w: 12, color: 0xc7ccd6, life: 3 });
          Sound.dash();
        }
        f.st.fireT = 1.8;
        f.st.damage += 0.05;
...

  {
    ...
    hp: 158, speed: 304, radius: 40, weapon: { kind: "blade", length: 240, width: 20, style: "katana" },
    spin: 6.4, damage: 10,
    ult: {
...

  {
    ...
    hp: 165, speed: 266, radius: 42, weapon: { kind: "blade", length: 150, width: 22, style: "sword" },
    spin: 4.6, damage: 6,
    ult: {
...

  {
    ...
    hp: 142, speed: 276, radius: 40, weapon: { kind: "gun", length: 175, width: 30, style: "railgun" },
    spin: 0, damage: 3.5,
    ult: {
      ...
      fire(f, e) {
        ...
        for (let i = 0; i < 12; i++) {
          ...
          e.spawnProjectile(f, "gateblade", f.x + Math.cos(base + a) * (f.def.radius + 26), f.y + Math.sin(base + a) * (f.def.radius + 26), base + a,
            { speed: 779, dmg: f.st.damage, l: 110, w: 13, color: 0x9fd8ff, life: 2.2 });
        }
      },
    },
    update(f, e, dt) {
      ...
      if (f.st.fireT <= 0) {
        ...
        if (t) {
          ...
          e.spawnProjectile(f, "gateblade", f.x + Math.cos(a) * (f.def.radius + 30), f.y + Math.sin(a) * (f.def.radius + 30), a,
            { speed: 950, dmg: f.st.damage, l: 110, w: 13, color: 0x9fd8ff, life: 1.8 });
          Sound.pew();
          // Shred ramps; Damage is always 3x Shred (as in his stat readouts)
          f.st.shred = Math.min(6, (f.st.shred ?? 2.8) + 0.11);
    ...
    stats: (f) => [
      { label: "Shred", value: () => f2(f.st.shred ?? 2.8) },
      { label: "Damage", value: () => f2(f.st.damage) },
    ],
  },

  {
    ...
    hp: 172, speed: 270, radius: 42, weapon: { kind: "blade", length: 195, width: 24, style: "dagger" },
    spin: 5.4, damage: 7.5,
    ult: {
...

  {
    ...
    hp: 150, speed: 289, radius: 40, weapon: { kind: "blade", length: 215, width: 26, style: "bolt" },
    spin: 6, damage: 10,
    ult: {
