...
export const ROSTER: CharacterDef[] = [
  {
    ...
    hp: 165, speed: 300, radius: 42, weapon: { kind: "blade", length: 240, width: 26, style: "greatsword" },
    spin: 5, damage: 5.5,
    ult: {
    ...
    onDealHit(f) { f.st.damage += 0.2; f.st.spinMult = f.st.damage / (2 * 2.5); }, // Damage = 2x Spin Speed
    stats: (f) => [
...

  {
    ...
    hp: 202, speed: 280, radius: 45, weapon: { kind: "blade", length: 215, width: 30, style: "axe" },
    spin: 4.2, damage: 4.2,
    ult: {
...

  {
    ...
    hp: 142, speed: 310, radius: 40, weapon: { kind: "gun", length: 130, width: 36, style: "revolver" },
    spin: 0, damage: 5.2,
    ult: {
      ...
      fire(f, e) {
        ...
        for (let i = 0; i < 6; i++) {
          ...
          e.spawnProjectile(f, "bullet", f.x + Math.cos(a) * (f.def.radius + 26), f.y + Math.sin(a) * (f.def.radius + 26), a,
            { speed: 1300, dmg: f.st.damage * 1.4, r: 9, color: 0xffd94d, life: 2 });
        }
        f.st.ammo = 6;
        Sound.pew();
      },
      ...
      if (f.st.shotT <= 0) {
        ...
        if (f.st.ammo > 0) {
          ...
          if (t) {
            ...
            e.spawnProjectile(f, "bullet", f.x + Math.cos(a) * (f.def.radius + 26), f.y + Math.sin(a) * (f.def.radius + 26), a,
              { speed: 1100, dmg: f.st.damage, r: 8, color: 0xffd94d, life: 2 });
            Sound.pew();
            f.st.ammo--;
            f.st.shotT = 0.6;
          }
...

  {
    ...
    hp: 135, speed: 290, radius: 40, weapon: { kind: "blade", length: 155, width: 22, style: "wand" },
    spin: 3.6, damage: 5,
    ult: {
      ...
      fire(f, e) {
        for (let i = 0; i < 8; i++) {
          ...
          e.spawnProjectile(f, "heart", f.x + Math.cos(a) * (f.def.radius + 16), f.y + Math.sin(a) * (f.def.radius + 16), a,
            { speed: 420, dmg: f.st.damage * 2, r: 15, color: 0xf284c1, life: 4, turn: 2.4 });
        }
        Sound.magic();
      },
    },
    update(f, e, dt) {
      ...
      if (f.st.castT <= 0) {
        ...
        if (t) {
          ...
          e.spawnProjectile(f, "heart", f.x + Math.cos(a) * (f.def.radius + 16), f.y + Math.sin(a) * (f.def.radius + 16), a,
            { speed: 520, dmg: f.st.damage * 2.2, r: 14, color: 0xf284c1, life: 4, turn: 3.4 });
          Sound.magic();
        }
        f.st.castT = 1.1;
      }
    },
    onDealHit(f) { f.st.atkspd = Math.min(3, (f.st.atkspd ?? 1) + 0.05); },
    stats: (f) => [{ label: "Attack Speed", value: () => f2(f.st.atkspd ?? 1) }],
  },

  {
    id: "monkeyking", name: "Monkey King", color: 0xc9a227, dark: 0x4d3a10, theme: "rings",
    hp: 158, speed: 305, radius: 41, weapon: { kind: "staff", length: 330, width: 24, stripes: 6 },
    spin: 5.2, damage: 4,
    ult: {
...

  {
    ...
    hp: 150, speed: 285, radius: 41, weapon: { kind: "blade", length: 205, width: 26, style: "iceblade" },
    spin: 4.6, damage: 5.5,
    ult: {
      name: "WORLD STASIS", gainDealt: 0.8, gainTaken: 0.7,
      fire(f, e) {
        for (const o of e.enemies(f)) {
          o.frozen = Math.max(o.frozen, 2.4);
    ...
    update(f, e, dt) {
      f.st.pulseT = (f.st.pulseT ?? 3) - dt;
      if (f.st.pulseT <= 0) {
        f.st.pulseT = 4.6;
        e.freezePulse(f, 250, 1.8);
      }
    },
    onDealHit(f) { f.st.damage += 0.1; },
    stats: (f) => [{ label: "Attack Speed", value: () => f2((f.st.spinMult ?? 1) * 2.26) }],
  },

  {
    id: "mach", name: "Mach", color: 0x2e55cc, dark: 0x101f4d, theme: "speed",
    hp: 128, speed: 430, radius: 37, weapon: { kind: "none" },
    spin: 0, damage: 6,
    ult: {
...

  {
    ...
    hp: 262, speed: 220, radius: 52, massMult: 3,
...

  {
    ...
    hp: 150, speed: 315, radius: 40, weapon: { kind: "blade", length: 235, width: 20, style: "katana" },
    spin: 6.2, damage: 6,
    ult: {
...

  {
    ...
    modDamage(f, base, e) {
      const crit = e.rng.next() * 100 < (f.st.crit ?? 15);
      return crit ? base * 2.5 : base;
    },
    onDealHit(f) { f.st.crit = Math.min(80, (f.st.crit ?? 15) + 2.5); },
    stats: (f) => [{ label: "Crit %", value: () => f2(f.st.crit ?? 15) }],
  },

  {
    id: "duelist", name: "Duelist", color: 0x5f6b76, dark: 0x232a31, theme: "plain",
    hp: 150, speed: 320, radius: 39, weapon: { kind: "blade", length: 225, width: 16, style: "rapier" },
    spin: 5.4, damage: 5,
    ult: {
      ...
      fire(f, e) {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          e.spawnProjectile(f, "bullet", f.x + Math.cos(a) * (f.def.radius + 20), f.y + Math.sin(a) * (f.def.radius + 20), a,
            { speed: 900, dmg: f.st.damage * 1.3, r: 7, color: 0xd8dce4, life: 1.6 });
        }
      },
    },
...

  {
    ...
    hp: 172, speed: 295, radius: 42, weapon: { kind: "blade", length: 260, width: 28, style: "chainsaw" },
    spin: 4.4, damage: 4.6,
    ult: {
...

  {
    ...
    hp: 158, speed: 275, radius: 42, weapon: { kind: "none" },
    spin: 0, damage: 4,
    ult: {
    ...
    update(f, e, dt) {
      ...
      if (f.st.fireT <= 0) {
        ...
        if (t) {
          ...
          e.spawnProjectile(f, "nail", f.x + Math.cos(a) * (f.def.radius + 30), f.y + Math.sin(a) * (f.def.radius + 30), a,
            { speed: 1050, dmg: f.st.damage * 2.2, l: e.rng.range(280, 420), w: 12, color: 0xc7ccd6, life: 3 });
          Sound.dash();
        }
        f.st.fireT = 0.7;
        f.st.damage += 0.05;
...

  {
    ...
    hp: 142, speed: 290, radius: 40, weapon: { kind: "gun", length: 175, width: 30, style: "railgun" },
    spin: 0, damage: 3.5,
    ult: {
      ...
      fire(f, e) {
        ...
        for (let i = 0; i < 12; i++) {
          ...
          e.spawnProjectile(f, "gateblade", f.x + Math.cos(base + a) * (f.def.radius + 26), f.y + Math.sin(base + a) * (f.def.radius + 26), base + a,
            { speed: 820, dmg: f.st.damage, l: 110, w: 13, color: 0x9fd8ff, life: 2.2 });
        }
      },
    },
    update(f, e, dt) {
      ...
      if (f.st.fireT <= 0) {
        ...
        if (t) {
          ...
          e.spawnProjectile(f, "gateblade", f.x + Math.cos(a) * (f.def.radius + 30), f.y + Math.sin(a) * (f.def.radius + 30), a,
            { speed: 1000, dmg: f.st.damage, l: 110, w: 13, color: 0x9fd8ff, life: 1.8 });
          Sound.pew();
          // Shred ramps; Damage is always 3x Shred (as in his stat readouts)
          f.st.shred = Math.min(4.5, (f.st.shred ?? 1.8) + 0.07);
    ...
    stats: (f) => [
      { label: "Shred", value: () => f2(f.st.shred ?? 1.8) },
      { label: "Damage", value: () => f2(f.st.damage) },
    ],
  },
...

  {
    ...
    hp: 150, speed: 305, radius: 40, weapon: { kind: "blade", length: 215, width: 26, style: "bolt" },
    spin: 5, damage: 6,
    ult: {
